...
def api_song_stage(request):
    ...
    if not song_stage:
        print("No song stage provided")
        return JsonResponse({"status": "failure", "error": "Song stage not provided"}, status=400)

    if not request.user or not request.user.is_authenticated:
        return JsonResponse({"status": "failure", "error": "User not authenticated"}, status=401)
    
    print(f"Updating song with ID: {song_id} to stage '{song_stage}'")
